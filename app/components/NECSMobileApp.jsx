...
import { Home, TrendingUp, Play, Trophy, MessageCircle, ArrowLeft, Search, Filter, Send, MoreVertical, Star } from 'lucide-react';
...
const NECSMobileApp = () => {
  ...
  const LiveScoresScreen = () => (
    <div className="flex flex-col h-full bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900">
      <div className="p-4 bg-gradient-to-r from-purple-600 to-pink-600">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          {['All', 'Valorant', 'Rocket League', 'Smash Bros'].map(game => (
            <button key={game} className="px-4 py-2 bg-white/20 backdrop-blur-sm rounded-full text-white text-sm whitespace-nowrap">
              {game}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {[
          { team1: 'Cloud9', team2: 'Sentinels', score1: 11, score2: 9, game: 'Valorant', status: 'LIVE', map: 'Haven' },
          { team1: 'NRG Esports', team2: '100 Thieves', score1: 8, score2: 13, game: 'Valorant', status: 'LIVE', map: 'Bind' },
          { team1: 'Team Liquid', team2: 'FaZe Clan', score1: 2, score2: 1, game: 'Rocket League', status: 'LIVE', time: '3:24' },
        ].map((match, idx) => (
          <div key={idx} className="bg-gradient-to-br from-slate-800 to-slate-700 rounded-2xl p-4 border border-purple-500/30 shadow-lg shadow-purple-500/20">
            <div className="flex items-center justify-between mb-3">
              <span className="px-3 py-1 bg-red-500 text-white text-xs font-bold rounded-full animate-pulse">
                • {match.status}
              </span>
              <span className="text-purple-400 text-sm font-semibold">{match.game}</span>
            </div>
            ...
            {match.map && (
              <div className="mt-3 pt-3 border-t border-slate-600/50">
                <span className="text-slate-400 text-sm">Map: <span className="text-purple-400">{match.map}</span></span>
              </div>
            )}
            {match.time && (
              <div className="mt-3 pt-3 border-t border-slate-600/50">
                <span className="text-slate-400 text-sm">Time: <span className="text-purple-400">{match.time}</span></span>
              </div>
            )}
          </div>
        ))}
    ...
    <div className="flex flex-col h-full bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900">
      <div className="p-4 bg-gradient-to-r from-blue-600 to-purple-600">
        <div className="flex items-center gap-3 mb-4">
          <h1 className="ml-2 text-2xl font-bold text-white flex-1">Player Stats</h1>
          <button className="p-2 bg-white/20 rounded-full backdrop-blur-sm">
            <Search size={20} className="text-white" />
          </button>
        </div>
        <div className="flex gap-2 overflow-x-auto pb-2">
          {['Top Players', 'K/D', 'Win Rate', 'MVP'].map(filter => (
            <button key={filter} className="px-4 py-2 bg-white/20 backdrop-blur-sm rounded-full text-white text-sm whitespace-nowrap">
              {filter}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {[
          { name: 'TenZ', team: 'Cloud9', kills: 284, deaths: 156, assists: 89, rating: 1.42, game: 'Valorant' },
          { name: 'Zekken', team: 'Sentinels', kills: 267, deaths: 178, assists: 102, rating: 1.38, game: 'Valorant' },
          { name: 's0m', team: 'NRG', kills: 245, deaths: 189, assists: 94, rating: 1.29, game: 'Valorant' },
          { name: 'Asuna', team: '100T', kills: 238, deaths: 195, assists: 87, rating: 1.24, game: 'Valorant' },
        ].map((player, idx) => (
          <div key={idx} className="bg-gradient-to-br from-slate-800 to-slate-700 rounded-xl p-4 border border-blue-500/30 shadow-lg">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-slate-400 text-sm">{player.team} • {player.game}</p>
              ...
              <div className="text-right">
                <div className="text-white font-bold text-lg">{player.kills}</div>
            ...
            <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-slate-600/50">
              ...
              <div className="text-center">
                <div className="text-purple-400 font-semibold">{(player.kills/player.deaths).toFixed(2)}</div>
  ...
  const MatchReplaysScreen = () => (
    <div className="flex flex-col h-full bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900">
      <div className="p-4 bg-gradient-to-r from-pink-600 to-orange-600">
        <h1 className="ml-2 text-2xl font-bold text-white mb-4">Match Replays</h1>
        <div className="relative">
          <input 
            type="text" 
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {[
          { title: 'Cloud9 vs Sentinels - Finals', thumbnail: 'C9', duration: '45:23', views: '152K', game: 'Valorant' },
          { title: 'NRG Clutch Moment', thumbnail: 'NRG', duration: '2:34', views: '89K', game: 'Valorant' },
          { title: 'Rocket League Championship', thumbnail: 'RL', duration: '38:12', views: '234K', game: 'RL' },
          { title: 'Smash Bros Top 8 Highlights', thumbnail: 'SSB', duration: '15:47', views: '178K', game: 'Smash' },
        ].map((replay, idx) => (
          <div key={idx} className="bg-gradient-to-br from-slate-800 to-slate-700 rounded-2xl overflow-hidden border border-pink-500/30 shadow-lg shadow-pink-500/20">
            <div className="relative h-48 bg-gradient-to-br from-pink-600 to-purple-600 flex items-center justify-center">
              <div className="text-white text-6xl font-bold opacity-30">{replay.thumbnail}</div>
            ...
            <div className="p-4">
              <h3 className="text-white font-bold text-lg mb-2">{replay.title}</h3>
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">{replay.views} views</span>
                <span className="px-2 py-1 bg-pink-500/20 text-pink-400 rounded">{replay.game}</span>
              </div>
            </div>
          </div>
        ))}
  ...
  const BracketScreen = () => (
    <div className="flex flex-col h-full bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900">
      <div className="p-4 bg-gradient-to-r from-green-600 to-teal-600">
        <h1 className="ml-2 text-2xl font-bold text-white mb-4">Tournament Bracket</h1>
        <div className="flex gap-2 overflow-x-auto pb-2">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="space-y-3">
              {[
                { team1: 'Cloud9', team2: 'NRG Esports', score1: 2, score2: 1, winner: 'team1' },
                { team1: 'Sentinels', team2: '100 Thieves', score1: 2, score2: 0, winner: 'team1' },
              ].map((match, idx) => (
                <div key={idx} className="bg-gradient-to-br from-slate-800 to-slate-700 rounded-xl p-4 border border-green-500/30">
                  <div className={`flex items-center justify-between mb-2 ${match.winner === 'team1' ? 'opacity-100' : 'opacity-60'}`}>
                    <span className="text-white font-semibold">{match.team1}</span>
                    <span className="text-white font-bold text-xl">{match.score1}</span>
                  </div>
                  <div className="border-t border-slate-600/50 my-2"></div>
                  <div className={`flex items-center justify-between ${match.winner === 'team2' ? 'opacity-100' : 'opacity-60'}`}>
                    <span className="text-white font-semibold">{match.team2}</span>
                    <span className="text-white font-bold text-xl">{match.score2}</span>
                  </div>
                </div>
              ))}
            </div>
  ...
  const ChatScreen = () => (
    <div className="flex flex-col h-full bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900">
      <div className="p-4 bg-gradient-to-r from-indigo-600 to-purple-600">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          {['General', 'Match Talk', 'Memes', 'LFG'].map(channel => (
            <button key={channel} className="px-4 py-2 bg-white/20 backdrop-blur-sm rounded-full text-white text-sm whitespace-nowrap">
              #{channel}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {[
          { user: 'ProGamer99', message: 'That clutch by TenZ was insane! 🔥', time: '2m ago', avatar: 'P' },
          { user: 'EsportsFan', message: 'Cloud9 looking strong this tournament', time: '5m ago', avatar: 'E' },
          { user: 'ValLover', message: 'Anyone else hyped for the finals tomorrow?', time: '8m ago', avatar: 'V' },
          { user: 'GameMaster', message: 'Sentinels vs Cloud9 is going to be epic!', time: '12m ago', avatar: 'G' },
          { user: 'NashvilleNerd', message: 'Who else is here in Nashville? This event is amazing!', time: '15m ago', avatar: 'N' },
        ].map((msg, idx) => (
